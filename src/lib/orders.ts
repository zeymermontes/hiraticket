...
export interface OrderItem { id: string; name: string; qty: number; unit_price: number; subtotal: number }
...
export async function getOrderDetail(orderId: string): Promise<OrderDetail | null> {
  ...
  const [{ data: items }, { data: notes }, { data: events }] = await Promise.all([
    supabase.from("order_items").select("id, name, qty, unit_price, subtotal").eq("order_id", orderId),
